...
$i=0;
$pdf->SetFont( 'Arial', '', 6 );
$Query = mysqli_query($link,"SELECT HOUR(hour) as hour, AVG(LFR) AS LFR, AVG(OFR) AS OFR, AVG(WFR) AS WFR, AVG(GFR) AS GFR, AVG(WCUT) AS WCUT, AVG(GVF) AS GVF, AVG(TMP) AS TMP, AVG(PRE) AS PRE FROM `minutedata` GROUP BY HOUR(hour)");
while($Row = mysqli_fetch_array($Query)){
  $pdf -> SetX(17);

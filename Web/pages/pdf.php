...
$pdf->Ln(15);
$Query2 = mysqli_query($link,"SELECT HOUR(hour) as hour, AVG(LFR) AS LFR, AVG(OFR) AS OFR, AVG(WFR) AS WFR, AVG(GFR) AS GFR, AVG(WCUT) AS WCUT, AVG(GVF) AS GVF, AVG(TMP) AS TMP, AVG(PRE) AS PRE FROM `minutedata` WHERE id >= $Row_Data[start_id] AND id <= $Row_Data[stop_id] GROUP BY HOUR(hour)");
